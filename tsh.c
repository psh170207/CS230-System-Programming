...
void eval(char *cmdline) 
{
	...
	pid_t cfpid; //process id of current foreground job
	...
	//ignore more,less,vi and emacs command
	if(!strcmp(argv[0],"more")) return;
	if(!strcmp(argv[0],"less")) return;
	if(!strcmp(argv[0],"vi")) return;
	if(!strcmp(argv[0],"emacs")) return;
	...
	if(!builtin_cmd(argv)){
		if(!bg){
			...
			if(cfpid!=0)
				waitfg(cfpid);
			state=FG;
		} // if bg=0, parent should wait until current foreground process would be terminated
		else{
			state=BG;
		}
		sigemptyset(&mask);
		sigaddset(&mask,SIGCHLD);
		sigprocmask(SIG_BLOCK,&mask,&prev_mask); //block SIGCHLD before fork()
		if((pid = fork())==0){ /* Child */
...
void do_bgfg(char **argv) 
{
	...
	if(argv[1][0]=='%'){ // second input argument is <job> and if it starts with '%' => jid
		...
		if(target == NULL){
			//unix_error("getjobjid error"); or
			printf("%s%d: No such job\n","%",jid);
			exit(1);
		}
		if(target->state != ST && target->state != BG){
			printf("%s%d: Not stopped or not a background job!\n","%",jid);
			exit(1);
		}
		pid = target->pid;
	}
		...
		if(target->state != ST && target->state != BG){
			printf("%d: Not stopped or not a background job!\n",pid);
			exit(1);
		}
	}
	...
	if(!strcmp(argv[0],"fg")){
		cfpid = fgpid(jobs);
		if(cfpid==0){ // no current foreground job
			kill(SIGCONT,pid); // continue the job
			target->state = FG; // set to foreground job
			exit(0);
		}
		else{
			waitfg(cfpid);//wait for current foreground job
			kill(SIGCONT,pid);
			target->state = FG;
			exit(0);
		}
...
void sigint_handler(int sig) 
{
    ...
    cfpid = fgpid(jobs);
	kill(-cfpid,SIGQUIT);//send SIGQUIT to foreground job.
...
void sigtstp_handler(int sig) 
{
	...
	cfpid = fgpid(jobs);
	Signal(SIGTSTP,SIG_DFL); //set SIGTSTP's action to  default action 
	kill(-cfpid,SIGTSTP);
	Signal(SIGTSTP,sigtstp_handler); // recover SIGTSTP's action
